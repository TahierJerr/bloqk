    ...
    return (
        <div className="flex flex-col gap-4">
            <p className="text-sm leading-relaxed text-muted-foreground">
                Goedgekeurd! Rond de betaling af, daarna zetten we {salonName}{" "}
                live.
            </p>
            {payment ? (
                <>
                    ...
                    {failureNote && (
                        <p className="text-sm font-medium text-destructive">{failureNote}</p>
                    )}
                    {inProgress ? (
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Loader2 className="size-4 animate-spin" />
                            We wachten op de bevestiging van je betaling. Dit kan even
                            duren, je hoeft niets te doen.
                        </div>
                    ) : (
                        <div>
                            <Button onClick={onPay} disabled={busy} className="cursor-pointer">
                ...
                <p className="text-sm leading-relaxed text-muted-foreground">
                    Maatwerk heeft geen vaste prijs; we nemen contact met je op
