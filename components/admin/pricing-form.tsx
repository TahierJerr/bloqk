...
const FIELDS: {
  ...
  {
    name: "websiteBase",
    label: "Website, vanafprijs",
    hint: "De prijs die je communiceert in het orderformulier.",
  },
  {
    name: "websiteSplitTotal",
    label: "Website, gespreid (totaal)",
    hint: "Totaalbedrag bij betalen in 12 maandtermijnen.",
  },
  {
    name: "websiteUpfront",
    label: "Website, in één keer",
    hint: "Bedrag bij de jaardeal (website vooraf betaald).",
  },
  {
    name: "subMonthly",
    label: "Abonnement, per maand",
    hint: "Dashboard + online boekingen, per maand.",
  },
  {
    name: "subYearly",
    label: "Abonnement, per jaar",
    hint: "Jaarprijs; reken zelf de korting (bijv. 2 maanden gratis) in.",
  },
  {
    name: "hostingShare",
    label: "Waarvan hosting (p/mnd)",
    hint: "Informatieve uitsplitsing voor de klant.",
  },
  {
    name: "softwareShare",
    label: "Waarvan software (p/mnd)",
    hint: "Informatieve uitsplitsing voor de klant.",
  },
];
...
export function PricingForm({ pricing }: { pricing: Pricing }) {
    ...
    try {
      const candidate = Object.fromEntries(
        Object.entries(values).map(([key, value]) => [
          key,
          toCents(value as string),
        ]),
      ) as unknown as Pricing;
      return Object.values(candidate).some((v) => Number.isNaN(v))
        ? null
        : candidate;
    } catch {
      return null;
  ...
  const monthlyPlan = preview
    ? computePaymentPlan(
        { package: "Wij bouwen het", billing: "monthly" },
        preview,
      )
    : null;
  const yearlyPlan = preview
    ? computePaymentPlan(
        { package: "Wij bouwen het", billing: "yearly" },
        preview,
      )
    : null;
          ...
          {monthlyPlan && preview ? (
            <>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                {formatEuro(preview.subMonthly)} abonnement. Na 12 maanden nog{" "}
                {formatEuro(preview.subMonthly)}/mnd.
              </p>
            </>
          ) : (
